  ...
  useEffect(() => {
    if (address) {
      ...
      .then(async (result: any) => {
        console.log("result: ", result.gf_sp_list_objects_by_bucket_name_response)
        const { statusCode, body } = result
        if (statusCode == 200 && Array.isArray(body.gf_sp_list_objects_by_bucket_name_response.objects)) {
          const t = body.gf_sp_list_objects_by_bucket_name_response.objects
            .map((item: any) => {
              return {
                ...
                objectInfo: {
                  ...
                  localVirtualGroupId: item.object_info.local_virtual_group_id,
